...
const UserDashboard = () => {
  ...
  const { user, isLoading } = useAuth(true, ['user']);
                ...
                <div className="space-y-3 sm:space-y-4">
                  {recommendedServices.map((service, index) => (
                    <div key={index} className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors cursor-pointer">
                      ...
                      <Button size="sm" variant="outline" className="border-2 border-[#1EC6D9] text-[#1EC6D9] hover:bg-[#1EC6D9] hover:text-white rounded-full text-xs flex-shrink-0">
                        Book
                      </Button>
                    </div>
                  ))}
